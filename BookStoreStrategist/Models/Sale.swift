...
struct Sale: Identifiable, Equatable {
    ...
    static var example = Sale(id: UUID(), book: Book.example, quantity: 5, saleDate: Date(timeIntervalSinceNow: -7_200_200))
    ...
    static var higherWeekendThreeMonthsExamples: [Sale] = {
        let theeMonthsAgo = Calendar.current.date(byAdding: .month, value: -3, to: Date())!
        
        let exampleSales: [Sale] = (1...300).map { _ in
            let randomBook = Book.examples.randomElement()!
            let randomDate = Date.random(in: theeMonthsAgo...Date())
            ...
            // Apply Central Limit Theorem to approximate Gaussian distrubution
            var average: Int = 35
            switch weekday {
                case 1: average = 29
                case 2: average = 21
