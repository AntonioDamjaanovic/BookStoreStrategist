...
struct DetailBookSalesView: View {
    ...
    var body: some View {
        VStack(alignment: .leading) {
            ...
            Group {
                Text("You sold \(Text("\(salesViewModel.totalSales) books").bold().foregroundColor(.accentColor)) in the last 90 days.")
            }
            .padding(.vertical)
            ...
            Group {
                ...
                case .week:
                    WeekySalesChartView(salesViewModel: salesViewModel)
                case .month:
